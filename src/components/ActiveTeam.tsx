...
export default function ActiveTeam() {
  ...
  const handleAdd = async () => {
    ...
    if (!result) { toast.error('Pokemon no encontrado'); return; }
  ...
  const handleSendToCemetery = () => {
    ...
    play('death');
    toast('Pokemon enviado al cementerio');
        ...
        {EMPTY_SLOTS.map(slot => {
          ...
          return pokemon
            ? <PokemonCard
                ...
                onRemove={() => { removeFromTeam(pokemon.id); toast('Pokemon eliminado'); }}
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-gray-900 border border-white/20 rounded-2xl p-6 w-full max-w-sm shadow-2xl my-4">
            <h3 className="text-white font-bold text-lg mb-4">Agregar Pokemon</h3>
            <div className="space-y-3">
              <div>
                <label className="text-white/50 text-xs block mb-1">Nombre o N. Pokedex *</label>
                <PokemonAutocomplete
                  value={searchQuery}
              ...
              <div>
                ...
                {loadedStats || useCustomStats ? (
                  ...
                  <p className="text-white/20 text-xs text-center py-2">
                    Selecciona un Pokemon para ver sus stats
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  placeholder="ej: Golpe critico de Surf"
              ...
              <div>
                ...
                <input
                  className="w-full bg-black/40 text-white rounded-lg px-3 py-2 border border-red-900/30 outline-none placeholder:text-white/30"
                  placeholder="ej: Rival Gary - Gyarados"
  ...
  return (
    <div className="pokemon-slot occupied p-3 flex flex-col gap-2 relative">
      ...
      <div className="flex justify-center relative">
        ...
        {pokemon.isShiny && <span className="absolute top-0 right-4 text-sm">S</span>}
      ...
      {isEditing ? (
        <div className="space-y-1.5">
          ...
          <div className="flex gap-1">
            ...
            <button onClick={onCancelEdit} className="flex-1 py-1 rounded bg-white/10 hover:bg-white/20 text-white/60 text-xs">Cancelar</button>
          </div>
        </div>
      ) : (
        <>
